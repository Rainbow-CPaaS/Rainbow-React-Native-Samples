...
import {
  MyProfileInfoNavigationProp,
  MyProfileInfoRouteProp,
...
export const MyProfileInfo: React.FunctionComponent<IProps> = ({
  ...
  useEffect(() => {
    const connectedUserUpdated = eventEmitter.addListener(
      ...
      (eventData: IUser) => {
        if (eventData) setUser(eventData);
      },
    );
  ...
  return (
    <>
        ...
        <ImageBackground
          ...
          <HStack space={2} justifyContent="flex-start" p="10">
            ...
            <VStack p="3" space={3}>
              ...
              <Text fontSize="xs" fontWeight="500" ml="-0.5" mt="-1">
                {Strings.presenceOption[contact.presence]}
              </Text>
            </VStack>
          </HStack>
        </ImageBackground>
      ...
      <VStack space={3}>
        ...
        {contact.phoneNumbers.map((item: IPhoneNumber) =>
          renderPhoneItem(item),
        )}
        <HStack alignItems="center" px={5}>
          <Text fontSize="md" fontWeight="bold" color="lightBlue.600">
            {Strings.servicePlane} :{' '}
          </Text>
          {licenses.map((item: string) => (
            <Text fontSize="xs" fontWeight="500" key={item}>
              {item} ,{' '}
            </Text>
          ))}
        </HStack>
