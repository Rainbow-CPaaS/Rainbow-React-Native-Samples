...
export const ConferenceDelegateContainer: React.FunctionComponent<IProps> = ({
  ...
  const [selectedParticipantToDelegate, setSelectedParticipantToDelegate] =
    useState<IConferenceParticipants>();

  const onParticipantSelected = (selectedContact: IContact) => {
    if (selectedContact.jId !== selectedParticipantToDelegate?.jId) {
      setSelectedParticipantToDelegate(
        selectedContact as IConferenceParticipants,
      );
    } else {
  ...
  const onDelegatePress = () => {
    if (selectedParticipantToDelegate) {
      conferenceService.delegateConference(
        bubbleId,
        selectedParticipantToDelegate.participantId,
      );
      onClosePressed();
    }
  };
  ...
  const renderParticipantsListView = () => {
    const renderItems = ({item}: {item: IConferenceParticipants}) => {
      const style =
        item.jId === selectedParticipantToDelegate?.jId
          ? defaultModalStyle.selectedContactItem
          : defaultModalStyle.centeredView;
      return (
        <ContactCardView
          contact={item}
          onPressed={onParticipantSelected}
          style={style}
        />
      );
    };
  ...
  return (
    <Modal animationType="slide" transparent={true} visible={showDelegateView}>
      <View style={defaultModalStyle.centeredView}>
        <View style={defaultModalStyle.modalView}>
          <Text style={defaultModalStyle.modalTitleText}>
            {Strings.EndMeeting}
          </Text>
          <Text style={defaultModalStyle.modalMessageText}>
            {getMessageText()}
          </Text>
          {delegateParticipants.length > 0 && (
            <View style={defaultModalStyle.participantList}>
              {renderParticipantsListView()}
            </View>
          )}
          <View style={defaultModalStyle.viewContainer}>
            <TouchableOpacity onPress={onClosePressed}>
              <Text style={defaultModalStyle.modalOptionsText}>
                {Strings.cancel}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={onEndPress}>
              <Text style={defaultModalStyle.endMeetingText}>
                {Strings.EndMeeting}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              style={
                selectedParticipantToDelegate === undefined
                  ? {opacity: 0.2}
                  : {opacity: 1}
              }
              disabled={selectedParticipantToDelegate === undefined}>
              <Text style={defaultModalStyle.modalOptionsText}>
                {Strings.Delegate}
              </Text>
            </TouchableOpacity>
