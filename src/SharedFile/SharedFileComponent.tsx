...
import {
  SharedFileListNavigationProp,
  SharedFileListRouteProp,
...
export const SharedFileComponent: React.FunctionComponent<IProps> = ({
  ...
  useEffect(() => {
    ...
    const sharedFileWithPeerResult = eventEmitter.addListener(
      ...
      (eventData: IFile[]) => {
        logger.info(
          `GetAllSharedFileWithPeerResult with file count ${eventData.length}`,
        );
  ...
  const sortBySize = () => {
    sharedFileArray.sort((a: IFile, b: IFile) =>
      a.size
        .toString()
        .localeCompare(b.size.toString(), undefined, {numeric: true}),
    );
  };
  const sortByDate = () => {
    sharedFileArray.sort((a: IFile, b: IFile) => {
  ...
  return (
    <>
      <Header
        containerStyle={defaultStyle.headerStyle}
      ...
      <Modal
        ...
        <TouchableOpacity
          style={defaultStyle.centeredView}
          onPressOut={showModalView}>
