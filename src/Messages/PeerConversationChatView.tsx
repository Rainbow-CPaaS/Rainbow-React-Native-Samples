...
export const PeerConversationChatView: React.FunctionComponent<
  IPeerChatProps
> = ({route, navigation}) => {
  const {peerIsInvited, peerIsRoster, name, contact, jId} =
  ...
  const selectMenuItem = (value: string) => {
    switch (value) {
      ...
      case PeerConversationMenuOptions.SeeSharedFiles:
        navigation.navigate('SharedFileList', {
          peer: route.params.conversation,
        });
        break;
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      {viewInviteBtn && (
        <FloatButton
          title={Strings.addToMyNetwork}
          style={addButtonStyle}
          onPress={addToRoster(contact.corporateId)}
        />
      )}
      <MessageComponent
        peer={route.params.conversation}
