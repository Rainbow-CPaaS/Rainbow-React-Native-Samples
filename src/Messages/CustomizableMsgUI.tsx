...
export const sendingExtraView = (
  title: string,
  onClose: () => void,
  style: IStyleSendingExtraView,
  msg?: string,
) => (
  <View style={style.containerStyle}>
    ...
    <Icon
      name="close-circle"
      style={defaultInternalStyle.closeIconStyle}
      onPress={onClose}
    />
  </View>
);
...
export const selectedAttachedFilesView = (
  filesToUpload: IAttachedFile[],
  isLoading: boolean,
  onCancel: (file: IAttachedFile) => () => void,
) => {
  const files = filesToUpload.map(file => {
    return (
      <View
        style={defaultInternalStyle.selectedAttachedFilesContainer}
        key={file.uri}>
        {!isLoading && (
          <TouchableOpacity
            onPress={onCancel(file)}
            style={defaultInternalStyle.cancelUploadIcon}>
            <Icon name="close-circle" size={30} color="white" />
          </TouchableOpacity>
        )}
        {file.type === Attached.FilesLibrary
          ? fileAttachedView(file)
          : imageAttachedView(file)}
        {isLoading && (
          <ActivityIndicator
            size="large"
            color="blue"
            style={defaultInternalStyle.attachedLoading}
          />
        )}
      </View>
    );
  ...
  return (
    <View style={defaultInternalStyle.filesToUploadContainer}>
      <ScrollView horizontal={true}>{files}</ScrollView>
    </View>
  );
};

export const messageHeaderView = (
  icon: ReactNode,
  style: IStyleHeaderView,
  msgType: string,
) => {
  return (
    <View style={style.containerStyle}>
...
export const sendingMsgTypesIcon = () => (
  <FontAwesomeIcon
    style={defaultInternalStyle.msgActionStyle}
    name="exclamation"
  />
);

export const attachIcon = () => (
  <EntypoIcon name="attachment" style={defaultInternalStyle.msgActionStyle} />
);
  ...
  return (
    <View>
      <Image
        ...
        key={file.uri}
      />
      <Text style={defaultInternalStyle.fileName}>
        {fileName.slice(0, 10) + ' ...'}
      </Text>
    </View>
  );
  ...
  return (
    <View>
      <View style={defaultInternalStyle.deletedMessageView}>
        <Text style={defaultInternalStyle.deletedMessageText}>
...
export const repliedMessageView = (
  msg: string,
  user: string,
  anyProps: any,
) => {
...
const defaultInternalStyle = StyleSheet.create({
  ...
  selectedAttachedFilesContainer: {
